...
def api_vversion_auth_post(version, body=None):  # noqa: E501
    ...
    if connexion.request.is_json:
        ...
        user_data = (
            ...
            .filter(db.User.email == body.login, db.User.password == body.password)
            .all()
        )
...
def api_vversion_users_sso_post(version, body=None):  # noqa: E501
    ...
    if connexion.request.is_json:
        ...
        try:
            decoded_token = jwt.decode(body.app_token, secret_key, algorithms=["HS256"])
            if decoded_token["user"] != body.login:
                return Error(error="Unauthorized"), 401

            session = db.Session()
            user_data = session.query(db.User).filter(db.User.email == body.login).all()
